...
function EditProfile() {
    ...
    useEffect(() => {
        if (selectedState && INDIAN_LOCATIONS[selectedState]) {
            ...
            // Logic for edit mode: Only clear city if user manually changes state.
            // But Form.useWatch triggers on mount too.
            // If profile is loaded, selectedState matches profile.state.
            // We need to ensure city options are populated without clearing existing city.
            // The form control will hold the value. We just need to ensure valid option exists.

            // However, we want to clear city if user CHANGED state.
            // Simple check: check if current city value is in the new list. 
            // If it is (valid) or logic is tricky due to bilingual, just trust form valid.
            // Actually, for dropdown, if value is not in options, it shows ID/Key.

            // To prevent clearing on initial load:
            // Check if form is dirty/touched?
            // Or simpler: check if current form value for city exists in new options.
            const currentCity = form.getFieldValue('city');
            if (currentCity && !INDIAN_LOCATIONS[selectedState].includes(currentCity)) {
                // City invalid for this state, imply state changed by user
    ...
    // const maritalStatusOptions = getOptions('maritalStatusOptions', isHindi); // Unused variable
    // const complexionOptions = getOptions('complexionOptions', isHindi); // Unused variable
    // const bodyTypeOptions = getOptions('bodyTypeOptions', isHindi); // Unused variable
    // const dietOptions = getOptions('dietOptions', isHindi); // Unused variable
    // const manglikOptions = getOptions('manglikOptions', isHindi); // Unused variable
    // const familyTypeOptions = getOptions('familyTypeOptions', isHindi); // Unused variable
        ...
        try {
            ...
            const formattedData = {
                ...
                horoscope: {
                    ...
                    birthTime: profileData.horoscope?.birthTime ? dayjs(profileData.horoscope.birthTime, 'HH:mm') : null
                }
            };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (values.horoscope?.birthTime && typeof values.horoscope.birthTime.format === 'function') {
        ...
        <div style={{ padding: '24px 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <Link to={`/profiles/${id}`} style={{ color: '#A0153E' }}>
                    ...
                    <div>
                        ...
                        <Text type="secondary">Update details for matches and family sharing.</Text>
                    </div>
                </div>
                <Alert
                ...
                <Card style={{ marginTop: 24 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        {currentStep < STEPS.length - 1 ? (
                            ...
                            <Button type="primary" onClick={handleSubmit} loading={saving} icon={<CheckOutlined />}>
                                Update Profile
                            </Button>
                        )}
                    </div>
